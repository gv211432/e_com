...
const ShowProduct = () => {
  ...
  const { data, extra, index } = state;
  // const { productDatas, setProductData } = useContext(UserContext);
  const [productData, setProductData] = useState({ ...data, img_url: data?.all_img_url[0] });
      ...
      <div className="row">
        <div className="col-12 col-sm-12 col-md-6 col-lg-6">
          ...
          <div className="row mb-3" style={{ maxHeight: "30rem" }}>
            ...
            >
              <br />
              {productData?.all_img_url?.map(url => (
                <img className='img-fluid mb-1'
                  src={`${config.baseURI}:${config.port}/api/common/files/${url}`}
            ...
            <center className="col-12 col-sm-10 bg-light p-1">
              <img className='img-fluid' style={{
                ...
                src={`${config.baseURI}:${config.port}/api/common/files/${productData?.img_url}`}
                alt="" srcset="" />
            </center>
          </div>
          <div className="row d-sm-block d-md-none p-1 ps-0">
